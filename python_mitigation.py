...
DATA_PATH       = "final_merged_cleaned.csv"
SHAP_CSV_PATH   = "shap_values.csv"          # from shap_llm_explainer.py

FAIR_MODEL_PATH = "fair_model.joblib"         # output -- saved fair model
REPORT_PATH     = "mitigation_report.json"    # output -- full metrics report
CHART_PATH      = "fairness_comparison.png"   # output -- side-by-side chart
...
def split_data(X, y):
    ...
    # Sensitive feature vectors -- used by Fairlearn to group predictions
    sensitive_train = X_train[SENSITIVE_FEATURE]
    sensitive_test  = X_test[SENSITIVE_FEATURE]

    # Map 0/1 to labels for readable Fairlearn output
    sensitive_train = sensitive_train.astype(int).map({0: "Female", 1: "Male"})
    sensitive_test  = sensitive_test.astype(int).map({0: "Female", 1: "Male"})

    print(f"\n  Sensitive feature  : {SENSITIVE_FEATURE}")
    print(f"  Train gender split : {sensitive_train.value_counts().to_dict()}")
    print(f"  Test  gender split : {sensitive_test.value_counts().to_dict()}")
